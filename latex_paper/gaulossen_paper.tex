...
\textit{Day 3 - Post-Rain Clearing:} Morning clearing brought increased corvid activity (visual: 15+ Hooded Crows, 8+ Carrion Crows within 200m radius). Notable behavioral interaction: corvid alarm calling preceded waterfowl flush event at 09:15, suggesting inter-species communication (see Figure \ref{fig:field_deployment} for field observations documenting deployment conditions and peak flock activity).
...
\draw[brown!70!black, line width=3pt] (0,0) -- (18,0);
\fill[brown!10] (0,-0.8) rectangle (18,0);
\node[brown!70!black, font=\Large] at (9,-1.2) {\textbf{Ground} (flat, open terrain)};

% Water surface (left side) - larger
\draw[blue!70, line width=3pt] (0,1) -- (5,1);
\fill[cyan!10] (0,0.2) rectangle (5,1);
\node[blue!80, font=\Large] at (2.5,1.8) {\textbf{Water Surface}};

% AudioMoth (right side) - larger
\fill[red!80] (16,2.5) rectangle (16.5,3.2);
\draw[red!80, line width=3pt] (16.25,3.2) -- (16.25,4.2);
\fill[red!90] (16.25,4.2) circle (0.25);
\node[above, red!90!black, font=\huge] at (16.25,4.7) {\textbf{AudioMoth}};
\node[below, font=\Large] at (16.25,2.2) {1.5m height};

% Bird 1 - Graylag Goose on water (left)
\fill[orange!80] (2,2) circle (0.25);
\node[above, orange!90!black, font=\Large] at (2,2.5) {\textbf{Graylag Goose}};
\node[below, orange!80, font=\large] at (2,1.4) {on water surface};

% Bird 2 - Hooded Crow (elevated, well to the left)
\fill[gray!70] (4,6) circle (0.22);
\node[above, gray!80!black, font=\Large] at (4,6.7) {\textbf{Hooded Crow}};
\node[below, gray!70, font=\large] at (4,5.3) {perched 4m};

% Bird 3 - Great Snipe flying (middle)
\fill[brown!70] (9,5.5) circle (0.22);
\node[above, brown!80!black, font=\Large] at (9,6.2) {\textbf{Great Snipe}};
\node[below, brown!70, font=\large] at (9,4.8) {in flight};

% DIRECT PATHS (clear, bold, well-spaced labels)
\draw[->, orange!70, line width=3pt, dashed] (2,2) -- (16,4.1);
\node[above, orange!80, font=\Large] at (9,3.5) {\textbf{Direct Path: 180m}};

\draw[->, gray!60, line width=3pt, dashed] (4,6) -- (16,4.3);
\node[above, gray!70, font=\Large] at (10,5.5) {\textbf{Direct: 150m}};

\draw[->, brown!60, line width=3pt, dashed] (9,5.5) -- (16,4.4);
\node[above, brown!70, font=\Large] at (12.5,5.2) {\textbf{Direct: 100m}};

% REFLECTED PATH from Graylag (water reflection) - clearer geometry
\draw[->, orange!40, line width=2.5pt, dotted] (2,2) -- (4.5,0.95);
\draw[->, orange!40, line width=2.5pt, dotted] (4.5,1.05) -- (15.8,3.9);
\fill[orange!60] (4.5,1) circle (0.18);
\node[below, orange!60, font=\Large] at (4.5,0.2) {\textbf{Water Reflection Point}};

% REFLECTED PATH from Crow (ground reflection)
\draw[->, gray!40, line width=2.5pt, dotted] (4,6) -- (10,0.1);
\draw[->, gray!40, line width=2.5pt, dotted] (10,0.1) -- (15.9,4);
\fill[gray!60] (10,0.1) circle (0.18);
\node[below, gray!60, font=\Large] at (10,-0.9) {\textbf{Ground Reflection Point}};

% Atmospheric fog layer - higher up
\draw[gray!40, line width=2.5pt, dashed] (0,8) -- (18,8);
\node[gray!70, font=\Large] at (16,8.8) {\textbf{Fog Layer}};
\node[gray!60, font=\large] at (16,8.3) {(10-20$\mu$m droplets)};

% Scattering particles
\foreach \x in {2,5,8,11,14,17} {
    \fill[gray!30] (\x,7.8) circle (0.12);
}
\node[gray!60, font=\large] at (9,7.2) {Rayleigh scattering};

% Absorption annotation (moved to avoid overlap)
\node[red!70, font=\Large, align=center] at (1.5,5) {\textbf{Atmospheric}\\[-0.1cm]\textbf{Absorption}\\[-0.1cm]$\alpha = 0.02$ dB/m\\[-0.1cm](@ 5kHz)};

% Legend (bottom right, larger, clearer spacing)
\draw[line width=3pt, dashed, orange!70] (12,0.5) -- (13.5,0.5);
\node[right, font=\Large] at (13.7,0.5) {\textbf{Direct path}};

\draw[line width=2.5pt, dotted, orange!40] (12,-0.3) -- (13.5,-0.3);
\node[right, font=\Large] at (13.7,-0.3) {\textbf{Reflected path}};
...
\caption{AudioMoth deployment on fence post, 100m from wetland edge}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{07_wetland_waterfowl_dramatic.jpg}
\caption{Challenging weather conditions: rain, fog, low visibility}
\end{subfigure}
...
\caption{Large Graylag Goose flock (peak event: 620 calls/91min)}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
...
\caption{\textbf{Field observations documenting deployment conditions and peak flock activity.} (a) Equipment mounted 1.5m height with rain shield providing 48.8 hours continuous recording (13-15 October 2025). AudioMoth positioned for unobstructed sight lines to primary wetland congregation areas. (b) Challenging atmospheric conditions dominated recording period: 80\% rain/fog coverage, 7-11°C, visibility often $<$500m, requiring extensive audio enhancement (see Figure \ref{fig:processing_comparison}). (c) Peak Graylag Goose flock activity Day 1 (16:00-17:26): visual field estimate 200+ individuals, 620 vocalizations quantified by BirdNET post-analysis, demonstrating intensive contact calling behavior. This 91-minute event represents largest continuous flock vocalization period detected during study. (d) Open wetland habitat with flat terrain providing favorable sound propagation conditions: 50-100m detection range (warblers), 200-500m (geese/cranes). Mountain backdrop (Dovrefjell range) visible during rare clear periods.}
...
% Water surface
\draw[blue!70, line width=3pt] (0,0) -- (12,0);
\fill[cyan!10] (0,-0.3) rectangle (12,0);
\node[blue!80, font=\Large] at (6,-0.6) {\textbf{Water Surface}};

% Source (Graylag Goose)
\fill[orange!80] (1,0.5) circle (0.2);
\node[above, orange!90!black, font=\Large] at (1,0.9) {\textbf{Source}};
\node[below, orange!80, font=\normalsize] at (1,0) {$h_s = 0.3$ m};

% Microphone (AudioMoth)
\fill[red!80] (11,3) circle (0.2);
\node[above, red!90!black, font=\Large] at (11,3.4) {\textbf{AudioMoth}};
\node[below right, red!80, font=\normalsize] at (11,2.6) {$h_m = 1.5$ m};

% Reflection point
\fill[orange!60] (6,0) circle (0.15);
\node[below, orange!70, font=\large] at (6,-0.8) {\textbf{Reflection Point}};

% Direct path
\draw[->, orange!70, line width=3pt, dashed] (1,0.5) -- (11,3);
\node[above, orange!80, font=\Large] at (6,2.2) {\textbf{Direct: } $L_d$};

% Reflected path (source to reflection point)
\draw[->, orange!40, line width=2.5pt, dotted] (1,0.5) -- (6,0);
\node[below, orange!60, font=\large] at (3.5,0.6) {$L_{r1}$};

% Reflected path (reflection point to microphone)
\draw[->, orange!40, line width=2.5pt, dotted] (6,0) -- (11,3);
\node[above, orange!60, font=\large] at (8.5,1.8) {$L_{r2}$};

% Angle annotations
\draw[gray, dashed] (6,0) -- (6,1.5);
\draw[gray, ->] (6,0.8) arc (90:155:0.8);
\node[gray, font=\normalsize] at (5.3,1.2) {$\theta_i$};
\draw[gray, ->] (6,0.8) arc (90:25:0.8);
\node[gray, font=\normalsize] at (6.7,1.2) {$\theta_r$};
\node[gray, font=\normalsize] at (9,0.8) {$\theta_i = \theta_r$ (specular)};

% Distance annotation
\draw[<->, brown!70, line width=1.5pt] (1,-1.5) -- (11,-1.5);
\node[below, brown!80, font=\Large] at (6,-1.7) {Horizontal distance: $r = 180$ m};

% Path difference equation box
\node[draw, thick, fill=yellow!10, align=left, font=\large] at (6,5.5) {
\textbf{Path Length Difference:}\\
$\Delta L = (L_{r1} + L_{r2}) - L_d$\\
\\
For $h_s = 0.3$ m, $h_m = 1.5$ m, $r = 180$ m:\\
$\Delta L \approx 0.85$ m\\
\\
\textbf{Interference Condition:}\\
Constructive: $\Delta L = n\lambda$ (gain up to +6 dB)\\
Destructive: $\Delta L = (n + 0.5)\lambda$ (nulls)
};

% Frequency response inset (simplified)
\draw[thick] (0.5,3.5) rectangle (4.5,5.2);
\node[above, font=\large] at (2.5,5.3) {Frequency Response};

% Simplified frequency response curve
\draw[->] (0.7,3.7) -- (0.7,5) node[left, font=\small] at (0.7,4.35) {Gain (dB)};
\draw[->] (0.7,3.7) -- (4.3,3.7) node[below, font=\small] at (2.5,3.7) {Frequency (Hz)};

% Draw oscillating response showing interference pattern
\draw[blue!70, line width=2pt] (0.7,4.35)
  .. controls (1.2,4.8) and (1.5,4.8) .. (1.8,4.35)
  .. controls (2.1,3.9) and (2.4,3.9) .. (2.7,4.35)
  .. controls (3.0,4.8) and (3.3,4.8) .. (3.6,4.35)
  .. controls (3.9,3.9) and (4.0,3.9) .. (4.3,4.35);

% Mark nulls
\foreach \x in {1.8,2.7,3.6} {
    \fill[red] (\x,4.35) circle (0.05);
    \node[below, font=\tiny, red] at (\x,3.9) {null};
}

% Frequency labels
\node[font=\tiny] at (0.9,3.6) {0};
\node[font=\tiny] at (1.8,3.6) {600};
\node[font=\tiny] at (2.7,3.6) {1200};
\node[font=\tiny] at (3.6,3.6) {1800};
