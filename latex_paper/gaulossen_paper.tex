...
\caption{Field deployment documentation October 13-15, 2025. (a) Equipment mounted 1.5m height with rain shield providing 48.8 hours continuous recording. (b) Atmospheric conditions: 80\% rain/fog coverage, 7-11°C, visibility often $<$500m. (c) Peak flock activity Day 1 (16:00-17:26): visual estimate 200+ Graylag Geese, 620 vocalizations quantified post-analysis. (d) Open wetland habitat providing favorable sound propagation: 50-100m detection range (warblers), 200-500m (geese/cranes).}
...
\end{tikzpicture}
\caption{Site map showing deployment geometry. AudioMoth positioned 100m from wetland edge on fence post (1.5m height). Concentric circles indicate acoustic detection ranges: 50-100m for quiet species (warblers, thrushes), 200-500m for loud species (geese, cranes).}
\label{fig:site_map}
\end{figure}
...
% Ground layer
\draw[brown!70!black, line width=2pt] (0,0) -- (12,0);
\fill[brown!10] (0,-0.5) rectangle (12,0);
\node[brown!70!black, font=\large] at (6,-0.8) {\textbf{Ground} (flat, open terrain)};

% Water surface (left side)
\draw[blue!70, line width=2pt] (0,1) -- (4,1);
\fill[cyan!10] (0,0.2) rectangle (4,1);
\node[blue!80, font=\large] at (2,1.5) {\textbf{Water}};

% AudioMoth (right side)
\fill[red!80] (10.5,2) rectangle (10.9,2.5);
\draw[red!80, line width=2.5pt] (10.7,2.5) -- (10.7,3.2);
\fill[red!90] (10.7,3.2) circle (0.15);
\node[above, red!90!black, font=\Large] at (10.7,3.5) {\textbf{AudioMoth}};
\node[below, font=\large] at (10.7,1.7) {1.5m};

% Bird 1 - Graylag Goose on water
\fill[orange!80] (1.5,1.8) circle (0.18);
\node[above, orange!90!black, font=\large] at (1.5,2.1) {\textbf{Graylag Goose}};
\node[below, orange!80, font=\normalsize] at (1.5,1.4) {on water};

% Bird 2 - Hooded Crow (elevated, to the side - NOT in front!)
\fill[gray!70] (2.5,4.2) circle (0.16);
\node[above, gray!80!black, font=\large] at (2.5,4.5) {\textbf{Hooded Crow}};
\node[below, gray!70, font=\normalsize] at (2.5,3.8) {perched 4m};

% Bird 3 - Great Snipe flying
\fill[brown!70] (5.5,3.8) circle (0.16);
\node[above, brown!80!black, font=\large] at (5.5,4.1) {\textbf{Great Snipe}};
\node[below, brown!70, font=\normalsize] at (5.5,3.4) {flying};

% DIRECT PATHS (clear, bold)
\draw[->, orange!70, line width=2.5pt, dashed] (1.5,1.8) -- (10.3,3.1);
\node[above, orange!80, font=\large] at (6,2.5) {\textbf{Direct: 180m}};

\draw[->, gray!60, line width=2.5pt, dashed] (2.5,4.2) -- (10.3,3.3);
\node[above, gray!70, font=\large] at (6.5,3.8) {\textbf{Direct: 150m}};

\draw[->, brown!60, line width=2.5pt, dashed] (5.5,3.8) -- (10.4,3.3);
\node[above, brown!70, font=\large] at (8,3.6) {\textbf{Direct: 100m}};

% REFLECTED PATH from Graylag (water reflection)
\draw[->, orange!40, line width=2pt, dotted] (1.5,1.8) -- (3,0.95);
\draw[->, orange!40, line width=2pt, dotted] (3,1.05) -- (10.3,2.8);
\fill[orange!60] (3,1) circle (0.12);
\node[below, orange!60, font=\normalsize] at (3,0.5) {Water reflection};

% REFLECTED PATH from Crow (ground reflection)
\draw[->, gray!40, line width=2pt, dotted] (2.5,4.2) -- (6,0.1);
\draw[->, gray!40, line width=2pt, dotted] (6,0.1) -- (10.4,2.9);
\fill[gray!60] (6,0.1) circle (0.12);
\node[below, gray!60, font=\normalsize] at (6,-0.5) {Ground reflection};

% Atmospheric fog layer
\draw[gray!40, line width=1.5pt, dashed] (0,5.5) -- (12,5.5);
\node[gray!70, font=\large] at (10,6) {\textbf{Fog layer}};
\node[gray!60, font=\normalsize] at (10,5.7) {(10-20$\mu$m droplets)};

% Scattering particles
\foreach \x in {3,5,7,9,11} {
    \fill[gray!30] (\x,5.3) circle (0.08);
}
\node[gray!60, font=\normalsize] at (6,4.9) {Rayleigh scattering};

% Absorption annotation
\node[red!70, font=\large, align=center] at (11.5,1) {\textbf{Absorption}\\$\alpha = 0.02$ dB/m\\(@ 5kHz)};

% Legend (larger, clearer)
\draw[line width=2.5pt, dashed, orange!70] (0.3,7) -- (1.5,7);
\node[right, font=\large] at (1.6,7) {\textbf{Direct path}};

\draw[line width=2pt, dotted, orange!40] (0.3,6.5) -- (1.5,6.5);
\node[right, font=\large] at (1.6,6.5) {\textbf{Reflected path}};

\end{tikzpicture}
\caption{Acoustic propagation showing direct and reflected sound paths. (Left) Graylag Goose on water produces water-reflected path. (Middle-left) Hooded Crow elevated to side produces ground reflection. (Center) Great Snipe in flight. All direct paths (dashed) show distances to AudioMoth. Fog layer causes Rayleigh scattering (high-frequency attenuation $>$8kHz). Atmospheric absorption $\alpha \approx 0.02$ dB/m at 5kHz in humid conditions.}
